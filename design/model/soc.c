    ...
    const soc_conf_t *conf)
{
    ...
    u32 itcm_latency = conf->perf_sim ? ITCM_LATENCY : 0u;
    u32 dtcm_latency = conf->perf_sim ? DTCM_LATENCY : 0u;

    AXI4_IF_CONSTRUCT(soc, cpu_mm_, 2);
    AXI4_IF_CONSTRUCT(soc, ufshc_dma_, 4);
    ...
    EXT_IRQ_SIGNAL_IF_CONSTRUCT(soc, peri_ufshc_irq_itf, false, false);

    soc->cpu.mod.cycle = soc->mod.cycle;
    AXI4_MST_CONNECT(&soc->cpu, mm_, soc, cpu_mm_);
    ...
    soc->cpu.ext_irq_ins[3] = &soc->peri_ufshc_irq_itf;
    ...
    rv32g_conf_t rv32g_conf = {
        ...
        .mm_base = MM_BASE,
        .mm_size = MM_SIZE,
        .dma_nc_base = UFS_DMA_POOL_BASE,
        .dma_nc_size = conf->ufshc.en ? UFS_DMA_POOL_SIZE : 0u,
        .cfg_base = CFG_BASE,
        .cfg_size = CFG_SIZE,
        .peri_base = PERI_BASE,
        .peri_size = PERI_SIZE,
    ...
    soc->peri.ufshc_irq_out = &soc->peri_ufshc_irq_itf;
    AXI4_MST_CONNECT(&soc->peri, ufshc_, soc, ufshc_dma_);
    peri_conf_t peri_conf = {
        .base = PERI_BASE,
        .size = PERI_SIZE,
        .ufshc = conf->ufshc
    };
    peri_construct(&soc->peri, soc->mod.hier_name, "u_peri", &peri_conf);

    AXI4_SLV_ARR_CONNECT(&soc->mm_axi_mux, host_, 0, soc, cpu_mm_);
    AXI4_SLV_ARR_CONNECT(&soc->mm_axi_mux, host_, 1, soc, ufshc_dma_);
    AXI4_MST_CONNECT(&soc->mm_axi_mux, gst_, soc, mm_);
    soc->mm_axi_mux.mod.cycle = soc->mod.cycle;
    axi_mux_conf_t mm_axi_mux_conf = {
        .host_num = 2,
        .stg_fifo_depth = SOC_BUS_STG_FIFO_DEPTH,
        .ost_depth = SOC_BUS_OST_DEPTH
    };
    axi_mux_construct(&soc->mm_axi_mux, soc->mod.hier_name,
        "u_mm_axi_mux", &mm_axi_mux_conf);
...
void soc_reset(soc_t *soc)
{
    ...
    rv32g_reset(&soc->cpu);
    peri_reset(&soc->peri);
    axi_mux_reset(&soc->mm_axi_mux);
    axi_demux_reset(&soc->mm_axi_demux);

    AXI4_IF_RESET(soc, cpu_mm_);
    AXI4_IF_RESET(soc, ufshc_dma_);
    ...
    itf_reset(&soc->peri_ufshc_irq_itf);
}
...
void soc_clock(soc_t *soc)
{
    ...
    rv32g_clock(&soc->cpu);
    peri_clock(&soc->peri);
    axi_mux_clock(&soc->mm_axi_mux);
    axi_demux_clock(&soc->mm_axi_demux);

    AXI4_IF_DBG_CLOCK(soc, cpu_mm_);
    AXI4_IF_DBG_CLOCK(soc, ufshc_dma_);
    ...
    itf_dbg_clock(&soc->peri_ufshc_irq_itf);
}
...
void soc_free(soc_t *soc)
{
    ...
    rv32g_free(&soc->cpu);
    peri_free(&soc->peri);
    axi_mux_free(&soc->mm_axi_mux);
    axi_demux_free(&soc->mm_axi_demux);

    AXI4_IF_FREE(soc, cpu_mm_);
    AXI4_IF_FREE(soc, ufshc_dma_);
    ...
    itf_free(&soc->peri_ufshc_irq_itf);
}
