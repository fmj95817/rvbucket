...
/* Keep stale wrong-path fetches alive across more than 16 redirects. */
...
asm(
    ...
    "s_mode_entry:\n"
    "    lui tp, %hi(trap_count)\n"
    "    addi tp, tp, %lo(trap_count)\n"
    "    csrw sscratch, tp\n"
    "    li tp, 0x00178000\n"
    "    li t0, 0x100\n"
    "    csrc sstatus, t0\n"
...
int main(void)
{
    ...
    return trap_count == TRAP_TARGET && bad_scause == 0u &&
        bad_mcause == 0u &&
        *(volatile uint32_t *)USER_DATA_PA == TRAP_TARGET ? 0 : 1;
}
