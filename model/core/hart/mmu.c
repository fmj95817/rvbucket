...
{
    if (mmu->pte_req_pending || itf_fifo_full(mmu->pa_d_bti_req_mst)) {
        return;
    }
    ...
    req.data = 0;
    req.strobe = 0xf;
    itf_write(mmu->pa_d_bti_req_mst, &req);
    mmu->pte_req_pending = true;
}

static void mmu_recv_pte_rsp(mmu_t *mmu)
{
    if (!mmu->pte_req_pending || itf_fifo_empty(mmu->pa_d_bti_rsp_slv)) {
        return;
    }

    bti_rsp_if_t rsp;
    itf_read(mmu->pa_d_bti_rsp_slv, &rsp);
...
{
    if (!mmu->busy || !mmu->is_inst) {
        mmu_forward_rsp(mmu, mmu->pa_i_bti_rsp_slv, mmu->va_i_bti_rsp_mst);
    }

    if (!mmu->busy || (mmu->is_inst && !mmu->pte_req_pending)) {
